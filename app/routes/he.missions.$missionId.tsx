...
export default function HeMissionPage({ loaderData, params }: Route.ComponentProps) {
  ...
  const handleCommentSave = () => {
    ...
    if (commentTarget.scope === "main") {
      ...
      const meta = linkedMissionMeta.get(commentTarget.parentLinkId);
      if (!meta) {
    ...
    <>
                    ...
                    {isPreview && selectedInstructionId === instruction.id && (
                      <div className={styles.editInstructionRow}>
                        ...
                        <button
                          ...
                          title={`הוסף תגובה מעל הוראה ${instruction.id}`}
                        >
                          💬 תגובה
                        </button>
                        {!isTemp && (
                          ...
                          >
                            ...
                            {isPreview && linkedSelected && (
                              <div className={styles.editInstructionRow}>
                                ...
                                <button
                                  className={styles.commentInstructionButton}
                                  onClick={() => openCommentDialog("linked", linkedInstruction.id, instruction.id)}
                                  title={`הוסף תגובה מעל הוראה ${linkedInstruction.id}`}
                                >
                                  💬 תגובה
                                </button>
                                <instrStatusFetcher.Form method="post" className={styles.instrStatusForm}>
      ...
      {commentTarget && (
        <div className={styles.dialogOverlay} onClick={() => setCommentTarget(null)}>
          <div className={styles.dialogContent} onClick={(e) => e.stopPropagation()}>
            <div className={styles.dialogHeader}>
              <h2 className={styles.dialogTitle}>הוסף תגובה מעל ההוראה</h2>
              <button className={styles.dialogClose} onClick={() => setCommentTarget(null)}>
                ✕
              </button>
            </div>
            <div className={styles.formGroup}>
              <label className={styles.label}>טקסט התגובה</label>
              <textarea
